...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['product_id'], $_POST['author'], $_POST['image_link'], $_POST['url'])) {
        ...
        $stmt = $conn->prepare("UPDATE oc_product SET url = ?, product_owner = ?, picture = ? WHERE product_id = ?");
        $stmt->bind_param("sssi", $url, $author, $image_link, $product_id);
...
<!--            <li>Total "Study Set" products without shipping and with files: <?= $total_studyset_with_file ?></li>
            ...
            <form id="upload_form" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" enctype="multipart/form-data">
                <div class="form-group">
                    <label for="step_one"><span style="color:#007bff; font-weight:bold;">Step 1: </span>Publish the book as a non-shippable product on the Admin Site</label>
                </div>
                <br>
                <div class="form-group">
