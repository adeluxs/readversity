(function($) {
    ...
    let $deactivateForm  = jQuery('#pricing-table-deactivateForm');
    ...
    let deactivateFormSubmitted = localStorage.getItem('pricing-table-deactivateFormSubmitted');
    ...
    function submitForm(deactivate_options, id) {
        ...
        data.push({name: 'action', value: 'pricing-table-deactivate'});
        ...
        if (!deactivateFormSubmitted) {
            localStorage.setItem('pricing-table-deactivateFormSubmitted', true);
            deactivateFormSubmitted = true;

            jQuery.post('./controller/extension/module/elfsight_pricing_table.php', jQuery.param(data)).then(function(result) {
