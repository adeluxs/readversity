...
class Deactivate {
    ...
    public function __construct($data) {
        ...
        $this->app_slug = 'pricing-table';
        $this->app_name = 'Pricing Table';
    ...
    public function setDeactivateReasons() {
        return require ( (__DIR__) . '/pricing-table-assets/reasons.php' );
    }
...
class ControllerExtensionModuleElfsightPricingTable extends Controller {
    private $error = array();

    public $params = NULL;

    const ELFSIGHT_EMBED_URL = 'https://apps.elfsight.com/embed/pricing-table/?utm_source=portals&utm_medium=opencart&utm_campaign=pricing-table&utm_content=sign-up';

    public function index() {
        $this->load->language('extension/module/elfsight_pricing_table');
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            $this->model_setting_setting->editSetting('module_elfsight_pricing_table', $this->request->post);
        ...
        $data['action'] = $this->url->link('extension/module/elfsight_pricing_table', 'user_token=' . $this->session->data['user_token'], true);

        $data['cancel'] = $this->url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token'] . '&type=module', true);

        if (isset($this->request->post['module_pricing_table_status'])) {
            $data['module_pricing_table_status'] = $this->request->post['module_pricing_table_status'];
        } else {
            $data['module_pricing_table_status'] = $this->config->get('module_pricing_table_status');
        }

        $data['header'] = $this->load->controller('common/header');
        $data['column_left'] = $this->load->controller('common/column_left');

        $this->response->setOutput($this->load->view('extension/module/elfsight_pricing_table', $data));
    }

    protected function validate() {
        if (!$this->user->hasPermission('modify', 'extension/module/elfsight_pricing_table')) {
    ...
    public function setDeactivateReasons() {
        return require ( './controller/extension/module/pricing-table-assets/reasons.php' );
    }
    ...
    public function uninstall() {
        ...
        include './controller/extension/module/pricing-table-assets/popup.php';
    }
