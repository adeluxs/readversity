(function($) {
    ...
    let $deactivateForm  = jQuery('#twitter-feed-deactivateForm');
    ...
    let deactivateFormSubmitted = localStorage.getItem('twitter-feed-deactivateFormSubmitted');
    ...
    function submitForm(deactivate_options, id) {
        ...
        data.push({name: 'action', value: 'twitter-feed-deactivate'});
        ...
        if (!deactivateFormSubmitted) {
            localStorage.setItem('twitter-feed-deactivateFormSubmitted', true);
            deactivateFormSubmitted = true;

            jQuery.post('./controller/extension/module/elfsight_twitter_feed.php', jQuery.param(data)).then(function(result) {
