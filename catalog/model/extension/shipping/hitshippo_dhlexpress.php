...
class ModelExtensionShippinghitshippodhlexpress extends Model {
	function getQuote($address) {
		
		$this->load->language('extension/shipping/hitshippo_dhlexpress');
		
		if($this->config->get('shipping_hitshippo_dhlexpress_realtime_rates') == true)
		{
			$status = true;
		}
		else
		...
		if ($status) {
			...
			$dhl_packs		=	$this->hitshippo_get_dhl_packages( $get_product,$selected_currency );
			$total_value= 0;
			...
			if (!$this->config->get('shipping_hitshippo_dhlexpress_test')) {
				$url = 'https://xmlpi-ea.dhl.com/XMLShippingServlet';
			} else {
				$url = 'https://xmlpitest-ea.dhl.com/XMLShippingServlet';
			}
			
			$pieces = $this->hitshippo_get_package_piece($dhl_packs);
			$weight_unit = ($this->config->get('shipping_hitshippo_dhlexpress_weight') == true) ? 'LB' : 'KG';
			$dim_unit = ($this->config->get('shipping_hitshippo_dhlexpress_weight') == true) ? 'IN' : 'CM';
			$fetch_accountrates = ($this->config->get('shipping_hitshippo_dhlexpress_rate_type') == 'ACCOUNT') ? "<PaymentAccountNumber>" . $this->config->get('shipping_hitshippo_dhlexpress_account') . "</PaymentAccountNumber>" : "";
		
			$mailing_date = date('Y-m-d');
			$mailing_datetime = date('c');
			$origin_postcode_city = $this->hitshippo_get_postcode_city($this->config->get('shipping_hitshippo_dhlexpress_country_code'), $this->config->get('shipping_hitshippo_dhlexpress_city'), $this->config->get('shipping_hitshippo_dhlexpress_postcode'));
			//$total_value = $this->cart->get_total();
			$is_dutiable = ($address['iso_code_2'] == $this->config->get('shipping_hitshippo_dhlexpress_country_code') || $this->hitshippo_dhl_is_eu_country($this->config->get('shipping_hitshippo_dhlexpress_country_code'), $address['iso_code_2'])) ? "N" : "Y";
			if($address['iso_code_2'] == 'AT' && $this->config->get('shipping_hitshippo_dhlexpress_country_code') == 'CZ'){
				$is_dutiable = "N";
			}
			if($address['iso_code_2'] == 'NL' && $this->config->get('shipping_hitshippo_dhlexpress_country_code') == 'SE'){
				$is_dutiable = "N";
			}
			...
			$insurance_details = ($this->config->get('shipping_hitshippo_dhlexpress_insurance') == true) ? "<InsuredValue>". $total_value ."</InsuredValue><InsuredCurrency>". $this->config->get('config_currency') ."</InsuredCurrency>" : "";
			$additional_insurance_details = ($this->config->get('shipping_hitshippo_dhlexpress_insurance') == true)  ? "<QtdShp><QtdShpExChrg><SpecialServiceType>II</SpecialServiceType><LocalSpecialServiceType>XCH</LocalSpecialServiceType></QtdShpExChrg></QtdShp>" : ""; //
			$to_city = $address['city'];
			if ( $this->config->get('shipping_hitshippo_dhlexpress_translation') == true && !empty($this->config->get('shipping_hitshippo_dhlexpress_translation_key')) ) {
				if (!empty($to_city)) {
					if (!preg_match('%^[ -~]+$%', $to_city))			//Cheks english or not	/[^A-Za-z0-9]+/	
					{
						$response =array();
						try{
							$translate = new TranslateClient(['key' => $this->config->get('shipping_hitshippo_dhlexpress_translation_key')]);
							// Tranlate text
			...
			$payment_country = $this->config->get('shipping_hitshippo_dhlexpress_country_code');

			if ( !empty($this->config->get('shipping_hitshippo_dhlexpress_pay_con')) && $this->config->get('shipping_hitshippo_dhlexpress_pay_con') == "R" ) {
				$payment_country = $address['iso_code_2'];
			}elseif( !empty($this->config->get('shipping_hitshippo_dhlexpress_pay_con')) && $this->config->get('shipping_hitshippo_dhlexpress_pay_con') == "C" ) {
				if ( !empty($this->config->get('shipping_hitshippo_dhlexpress_cus_pay_con')) ) {
					$payment_country = $this->config->get('shipping_hitshippo_dhlexpress_cus_pay_con');
				}
			}
			...
			$xml .= '					<SiteID>'.$this->config->get('shipping_hitshippo_dhlexpress_key').'</SiteID>';
			$xml .= '					<Password>'.$this->config->get('shipping_hitshippo_dhlexpress_password').'</Password>';
			$xml .= '			</ServiceHeader>';
			$xml .= '		</Request>';
			$xml .= '		<From>';
			$xml .= '			<CountryCode>'.$this->config->get('shipping_hitshippo_dhlexpress_country_code').'</CountryCode>';
			...
			$result = utf8_encode(curl_exec($curl));
				...
				if(!empty($result))
				{
				 ...
				 $result = $xml;	
			if($this->config->get('shipping_hitshippo_dhlexpress_front_end_logs') == true)
			{
				echo "<pre>";
				echo '<h3>Request</h3>';
			...
			if ($result && !empty($result->GetQuoteResponse->BkgDetails->QtdShp)) {
				...
				foreach ($result->GetQuoteResponse->BkgDetails->QtdShp as $quote) {
					...
					$quote_data[$rate_code] = array(
							'code'         => 'hitshippo_dhlexpress.' . $rate_code,
...
//echo "<pre>";		//	print_r($result);		//	die();
		...
		//	}
			...
			$method_data = array(
				...
				'title'      => $title,
				'quote'      => $quote_data,
				'sort_order' => $this->config->get('shipping_hitshippo_dhlexpress_sort_order'),
				'error'      => $error
			);
		}
		return $method_data;
	...
	public function hitshippo_get_dhl_packages($package,$orderCurrency,$chk = false)
	{
		switch ($this->config->get('shipping_hitshippo_dhlexpress_packing_type')) {
			case 'weight_based' :
	...
	public function weight_based_shipping($package,$orderCurrency,$chk='')
		{
			$maximum_weight = ($this->config->get('shipping_hitshippo_dhlexpress_wight_b') !='') ? $this->config->get('shipping_hitshippo_dhlexpress_wight_b') : '50';
								
			if ( ! class_exists( 'WeightPack' ) ) {
				include_once 'class-hitshippo-dhl-weight-packing.php';
			}
			...
			if( !empty($errors) ){
				...
				foreach($packages as $package){//pre($package);
					...
					$group = array(
						...
						'Units' => ($this->config->get('shipping_hitshippo_dhlexpress_weight') == true) ? 'LBS' : 'KG'
					),
		...
		foreach ($package as $item_id => $values) {
			...
			if ($values['width'] && $values['height'] && $values['length']) {
				...
				$group['Dimensions'] = array(
					...
					'Units' => ($this->config->get('shipping_hitshippo_dhlexpress_weight') == true) ? 'IN' : 'CM'
				);
			}
			$group['packtype'] = $this->config->get('shipping_hitshippo_dhlexpress_per_item');
			$group['InsuredValue'] = $insurance_array;
	...
	private function hitshippo_get_package_piece($dhl_packages) {
		$pieces = "";
		if ($dhl_packages) {
