...
class ControllerExtensionModuleApimodule extends Controller {
    ...
    public function afterAoh(&$route, &$args, &$output) {
        if (!empty($args) && (count($args) >= 2)) {
            if (count($this->model_extension_module_apimodule->getOrderHistory($args[0])) == 1) {
    ...
    public function afterAddReview(&$route, &$args, &$output) {
        $this->sendNotifications([
            'actionId' => 2,
            'productId' => (int) $args[0], // $args[1] это массив с информацией о ревью, может ее тоже передавать?
        ]);
    }
    ...
    public function login()
    {
        ...
        if (
            isset($this->request->post['device_token'])
            && $this->request->post['device_token'] != ''
            && isset($this->request->post['os_type'])
            && $this->request->post['os_type'] != ''
        ) {
            $devices = $this->model_extension_module_apimodule->getUserDevices($user['user_id']);
            $matches = 0;
            ...
            if ($matches == 0) {
                $this->model_extension_module_apimodule->setUserDeviceToken(
                    $user['user_id'],
                    $_REQUEST['device_token'],
                    $_REQUEST['os_type'],
                    (isset($_REQUEST['device_name']) ? $_REQUEST['device_name'] : null)

                );
            } elseif ($matches > 0) {
                $this->model_extension_module_apimodule->updateUserDeviceTokenLastLogin(
                    $_REQUEST['device_token']
                );
            }
        }
    ...
    public function sendNotifications($data)
    {
        ...
        switch ($data["actionId"]) {
            case 0:
                $dataForNewOrder = $this->dataForNewOrder($data["orderId"], $data["orderStatusId"]);
                $msg = $dataForNewOrder["msg"];
                $msg_android = $dataForNewOrder["msg_android"];
                break;
            case 1:
                $dataForNewCustomer = $this->dataForNewCustomer($data["customerId"]);
                $msg = $dataForNewCustomer["msg"];
                $msg_android = $dataForNewCustomer["msg_android"];
                break;
            case 2:
                $dataForNewReview = $this->dataForNewReview($data["productId"]);
                $msg = $dataForNewReview["msg"];
                $msg_android = $dataForNewReview["msg_android"];
                break;
            case 3:
                $dataForOrderStatusChange = $this->dataForOrderStatusChange($data["orderId"], $data["orderStatusId"]);
                $msg = $dataForOrderStatusChange["msg"];
                $msg_android = $dataForOrderStatusChange["msg_android"];
                break;
        }
    ...
    private function dataForNewOrder($order_id, $order_status_id)
    {
        ...
        $msg = [
            ...
            'event_type' => $this->PUSH_ACTIONS[0],
        ];
        ...
        return [
            'msg' => $msg,
            'msg_android' => $msg_android,
        ];
    }

    private function dataForNewCustomer($customer_id)
    {
        ...
        $msg = [
            ...
            'event_type' => $this->PUSH_ACTIONS[1],
        ];
    ...
    private function dataForNewReview($product_id)
    {
        $this->load->model('catalog/product');
        $url = (empty($_SERVER['HTTPS']) ? 'http://' : 'https://') . $_SERVER['HTTP_HOST'];
        ...
        $msg = [
            ...
            'event_type' => $this->PUSH_ACTIONS[2],
        ];
    ...
    private function dataForOrderStatusChange($order_id, $order_status_id)
    {
        ...
        $msg = [
            ...
            'event_type' => $this->PUSH_ACTIONS[3],
        ];
    ...
    {
        $API_ACCESS_KEY = 'AAAAlhKCZ7w:APA91bGGTpgKQbktZAef-VD85nF8n-jPAeDaedFFTNbj849Etcf7WKZJWcZcl-2B4ttvwYrkqxTsLfifNZrMvbkx6V1zkWyTUKUSR6jWsIPInTK6eAE7nct1h2k09rxqSJJgOAQSjqDo';
        $headers = [
