...
class ControllerApiOrderAkaunting extends Controller
{
    public function index()
    {
        ...
        if (isset($this->request->post['filter_order_status_ids'])) {
            $filter_order_status_ids = $this->request->post['filter_order_status_ids'];
        } else {
            $filter_order_status_ids = '';
        }
        ...
        $filter_data = array(
            'filter_order_status'  => $filter_order_status_ids,
        ...
        $response['meta']['total'] = $this->model_extension_module_akaunting->getTotalOrders($filter_data);

        $results = $this->model_extension_module_akaunting->getOrders($filter_data);

        foreach ($results as $result) {
            if (empty($result['invoice_no'])) {
                $invoice_number = $this->model_extension_module_akaunting->createInvoiceNo($result);
            } else {
            ...
            $address_info = [
                ...
                'zone_code' => $result['payment_zone_code'],
                'country'   => $result['payment_country'],
            ];

            $address = $this->getCustomerAddress($address_info, $result['payment_address_format']);
        ...
        if (!isset($this->session->data['api_id'])) {
            ...
            if (!$json) {
                ...
                $order_data['payment_custom_field']   =
                    (isset($this->session->data['payment_address']['custom_field']) ?
                ...
                if ($this->cart->hasShipping()) {
                    ...
                    $order_data['shipping_custom_field']   =
                        (isset($this->session->data['shipping_address']['custom_field']) ?
            ...
            if ($order_info) {
                ...
                if (!$json) {
                    ...
                    if (isset($this->request->post['affiliate_id'])) {
                        ...
                        $affiliate_info =
                            $this->model_account_customer->getAffiliate($this->request->post['affiliate_id']);
