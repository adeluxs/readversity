...
// Fetch products that do not require shipping and have uploaded files 

$query = "SELECT p.product_id, pd.name AS product_name FROM oc_product p JOIN oc_product_description pd ON p.product_id = pd.product_id WHERE p.shipping = 0 AND (p.url IS NOT NULL AND p.url <> '')";
